...
@Component({
  ...
  styleUrls: ['./report-lost-item.css'],
})
export class ReportLostItem implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (id && mode === 'edit') {
        ...
        this.reportForm.patchValue({ dateLost: today });
      }
    });
  }
    ...
    this.lostItemService.getById(id).subscribe({
      next: (response) => {
        ...
        if (response.isSuccess && response.data) {
          ...
          const dateLost = item.dateLost ? new Date(item.dateLost).toISOString().split('T')[0] : '';
          ...
          canvas.toBlob(
            (blob) => {
              ...
              const croppedFile = new File(
                [blob],
                this.makeDerivedFilename(file.name, 'framed-4x3'),
                {
                  type: 'image/jpeg',
  ...
  onSubmit(): void {
    ...
    const dateLostString = formValue.dateLost;
    const dateLostDateTime = dateLostString
      ? new Date(dateLostString).toISOString()
      : new Date().toISOString();

    if (this.isEditMode && this.editItemId) {
      // Update existing item
      const updateDto: UpdateLostItemDto = {
        ...
        dateLost: dateLostDateTime,
        contactInfo: formValue.contactInfo,
      };

      console.log('Updating lost item with DTO:', updateDto);

      this.lostItemService.update(this.editItemId, updateDto).subscribe({
        next: (response) => {
          console.log('Lost item update response:', response);
          if (response.isSuccess) {
            this.successMessage = response.message || 'تم تحديث العنصر المفقود بنجاح!';
            this.isSubmitting = false;

            // Scroll to top to show success message
            window.scrollTo({ top: 0, behavior: 'smooth' });

            // Navigate to my-lost-items after 2 seconds
            setTimeout(() => {
              this.router.navigate(['/my-lost-items']);
            }, 2000);
          } else {
            this.errorMessage = response.message || 'فشل تحديث العنصر المفقود';
            this.isSubmitting = false;
          }
        },
        error: (error) => {
      ...
      // Create new item
      const createDto: CreateLostItemDto = {
        category: formValue.category,
        title: formValue.title,
        description: formValue.description,
        imageUrl: this.imageUrlService.resolve(formValue.imageUrl || undefined) || undefined,
      ...
      console.log('Creating lost item with DTO:', createDto);

      this.lostItemService.create(createDto).subscribe({
        next: (response) => {
          console.log('Lost item creation response:', response);
          if (response.isSuccess) {
            this.successMessage = response.message || 'تم الإبلاغ عن العنصر المفقود بنجاح!';
            this.isSubmitting = false;
            ...
            // Navigate to matches or my items after 2 seconds
            setTimeout(() => {
              this.router.navigate(['/home']);
            }, 2000);
          } else {
            this.errorMessage = response.message || 'فشل الإبلاغ عن العنصر المفقود';
            this.isSubmitting = false;
          }
        },
        error: (error) => {
